...
public class DynamicServerResolver implements Resolver<DiscoveryResult> {
    ...
    public DiscoveryResult resolve(@Nullable Object key) {
        ...
        DiscoveryEnabledServer des = ensureDiscoveryEnabled(server);
        return new DiscoveryResult(des, loadBalancer.getLoadBalancerStats());
    }
    ...
    void onUpdate(List<Server> oldList, List<Server> newList) {
        ...
        List<DiscoveryResult> discoveryResults = Sets.difference(oldSet, newSet).stream()
                .map(server -> new DiscoveryResult(ensureDiscoveryEnabled(server), loadBalancer.getLoadBalancerStats()))
                .collect(Collectors.toList());
        listener.onChange(discoveryResults);
    }

    private DiscoveryEnabledServer ensureDiscoveryEnabled(Server server) {
        if (server instanceof DiscoveryEnabledServer discoveryEnabledServer) {
        ...
        try {
            ...
            if (server.getMetaInfo() != null) {
                try {
                    String a = server.getMetaInfo().getAppName();
                    if (a != null && !a.isEmpty()) {
                        appName = a;
                    }
                } catch (Throwable t) {
                    LOG.debug("Could not read appName from server metaInfo", t);
                }
            }
            ...
            return new DiscoveryEnabledServer(instanceInfo, false);
        } catch (Exception e) {
            LOG.warn("Could not wrap Server into DiscoveryEnabledServer, falling back to EMPTY", e);
            return new DiscoveryEnabledServer(InstanceInfo.Builder.newBuilder().setAppName("undefined").setHostName("undefined").setPort(-1).build(), false);
        }
    }
}
